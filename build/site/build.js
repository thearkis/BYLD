...
Beast.decl({
    App: {
        ...
        domInit: function fn() {
            ...
            const dividerOneElements = document.querySelectorAll('.Divider_One')
            ...
            const caseClientElements = document.querySelectorAll('.Case__client')
            const caseTitleElements = document.querySelectorAll('.Case__title')
            console.log('Found case client elements:', caseClientElements.length) // Debug log
            console.log('Found case title elements:', caseTitleElements.length) // Debug log
            
            // Animate Case__client first
            caseClientElements.forEach((element, index) => {
                setTimeout(() => {
                    element.classList.add('Case__client_loaded')
                    console.log(`Case client ${index} animated`) // Debug log
            ...
            caseTitleElements.forEach((element, index) => {
                setTimeout(() => {
                    ...
                    console.log(`Case title ${index} animated`) // Debug log
            ...
            const caseMetaItems = document.querySelectorAll('.CaseMeta__meta-item')
            console.log('Found case meta items:', caseMetaItems.length) // Debug log
            ...
            console.log('Case elements finish time:', caseElementsFinishTime) // Debug log
            
            // Animate CaseMeta__meta-item elements after Case elements finish
            caseMetaItems.forEach((element, index) => {
                setTimeout(() => {
                    element.classList.add('CaseMeta__meta-item_loaded')
                    console.log(`Case meta item ${index} animated at time:`, caseElementsFinishTime + (index * 200)) // Debug log
            ...
            setTimeout(() => {
                caseMetaItems.forEach((element, index) => {
                    if (!element.classList.contains('CaseMeta__meta-item_loaded')) {
                        console.log(`Fallback: showing meta item ${index}`) // Debug log
                        element.classList.add('CaseMeta__meta-item_loaded')
                    }
                })
            }, 5000)
            ...
            console.log('Found caseresult items:', caseresultItems.length) // Debug log
            
            // Calculate when CaseMeta items will finish
            const caseMetaFinishTime = caseElementsFinishTime + (caseMetaItems.length * 200) + 200
            console.log('Case meta finish time:', caseMetaFinishTime) // Debug log
            ...
            caseresultItems.forEach((element, index) => {
                ...
                setTimeout(() => {
                    ...
                    console.log(`Caseresult item ${index} animated at time:`, earlierStartTime + (index * 100)) // Debug log
            ...
            console.log('Found caseresult elements - titles:', caseresultTitleElements.length, 'texts:', caseresultTextElements.length)
            ...
            allCaseresultTextElements.forEach(element => {
                ...
                console.log('Setting up character swapping for:', text)
                ...
                function animateCaseresultLetters() {
                    console.log('Starting character animation for:', text)
                ...
                if (parentItem) {
                    ...
                    console.log(`Scheduling ${isTitleElement ? 'title' : 'text'} animation for item ${itemIndex} at ${totalDelay}ms:`, element.textContent)
                    setTimeout(() => {
                        console.log(`Starting character animation for item ${itemIndex}:`, element.textContent)
            ...
            // CaseAction scroll-triggered character swapping animation
            const caseActionElements = document.querySelectorAll('.CaseAction')
            console.log('Found CaseAction elements for scroll animation:', caseActionElements.length) // Debug log
            ...
            if (caseActionElements.length > 0) {
                ...
                const caseActionObserver = new IntersectionObserver((entries) => {
                    entries.forEach(entry => {
                        if (entry.isIntersecting && !entry.target.hasScrollTriggered) {
                            ...
                            triggerCaseActionAnimation(entry.target)
                            console.log('CaseAction scroll animation triggered') // Debug log
                            
                            // Start occasional repeating animations
                            startCaseActionRepeatingAnimation(entry.target)
                        }
                    })
                }, caseActionObserverOptions)
                
                // Function to trigger the character swapping animation (same as hover)
                function triggerCaseActionAnimation(element) {
                    if (element.isAnimating) return
                ...
                // Function to start occasional repeating animations
                function startCaseActionRepeatingAnimation(element) {
                    function scheduleNextAnimation() {
                        const randomDelay = 3000 + Math.random() * 4000 // 3-7 seconds
                        setTimeout(() => {
                            if (!element.isAnimating) { // Only animate if not already animating
                                triggerCaseActionAnimation(element)
                            }
                            scheduleNextAnimation() // Schedule the next one
                        }, randomDelay)
                    }
            ...
            scrollAnimationElements.forEach(({ selector, className }) => {
                ...
                console.log(`Found ${selector} elements:`, elements.length) // Debug log
                    ...
                    const elementObserver = new IntersectionObserver((entries) => {
                        entries.forEach(entry => {
                            if (entry.isIntersecting) {
                                ...
                                console.log(`${selector} animated`) // Debug log
                            } else {
                                // Element is not visible - reset animation
                                entry.target.classList.remove(className)
                                console.log(`${selector} reset`) // Debug log
                            }
                        })
                    }, observerOptions)
