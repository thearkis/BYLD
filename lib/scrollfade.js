...
ScrollFade.setupScrollObserver = function(config) {
    ...
    function handleScrollEnd() {
        isScrolling = false
        // Force a check after scrolling ends
    ...
    if ('ontouchstart' in window) {
        ...
        document.addEventListener('touchcancel', handleScrollEnd, { passive: true })
        
        // Also listen for scroll events during touch
        let lastScrollTop = window.pageYOffset
        let scrollCheckInterval
        
        function checkScrollDuringTouch() {
            const currentScrollTop = window.pageYOffset
            if (Math.abs(currentScrollTop - lastScrollTop) > 10) { // Scrolling threshold
                lastScrollTop = currentScrollTop
                
                // Check elements during scroll
                elements.forEach(element => {
                    const rect = element.getBoundingClientRect()
        ...
        document.addEventListener('touchmove', () => {
            if (!scrollCheckInterval) {
                scrollCheckInterval = setInterval(checkScrollDuringTouch, 50) // Check every 50ms during scroll
            }
        }, { passive: true })
    ...
    return {
        ...
        destroy: function() {
            ...
            if ('ontouchstart' in window) {
                ...
                document.removeEventListener('touchcancel', handleScrollEnd)
                document.removeEventListener('touchmove', checkScrollDuringTouch)
                if (scrollCheckInterval) {
                    clearInterval(scrollCheckInterval)
                }
            }
